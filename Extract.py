...
class Extractor():
    def __init__(self, url="", blockSize=3, image=False, keywords=''):
        self.url = url
    ...
    def getRawPage(self):
        ...
        try:
            ...
            response = session.get(self.url, timeout=None, headers=headers)
        except:
            return 404, 'NOT FOUND!'
        self.encoding = self.get_encoding(response.text)
    ...
    def processTags(self):
        ...
        self.body = re.sub(reTRIM.format("head"), "", re.sub(reTRIM.format("footer"), "", self.body))
    ...
    def processBlocks(self):
        ...
        for i in range(self.blockSize):
            self.cblocks = list(
                map(lambda x, y: (x + y) / self.blockSize, self.textLens[i: lines - 1 - self.blockSize + i],
                    self.cblocks))
    ...
    # 根据网页编码写文件
    def WriteFile(self, name='', content='', search_flag=''):
        try:
            with open(f'C:\\Users\\13182\\Desktop\\Work\\extract\\{search_flag}\\{self.keywords}\\{self.Replace(name)}.txt', 'w', encoding=self.encoding) as f1:
                f1.write(content)

            with open(f'C:\\Users\\13182\\Desktop\\Work\\extract\\{search_flag}\\doc_encoding.txt', 'a', encoding='UTF-8') as f2:
                f2.write(name + '\t' + self.encoding + '\n')
